...
from config import ImageConfig as _ImageConfigBase, load_image_config as _load_image_config


@dataclass
class ImageConfig(_ImageConfigBase):
    pass
...
def load_image_config(project_dir: Path, assets_dir: Path | None = None) -> ImageConfig:
    cfg = _load_image_config(project_dir, assets_dir)
    return ImageConfig(
        api_url=cfg.api_url,
        api_key=cfg.api_key,
        model=cfg.model,
    ...
    *,
    ...
    if to_generate:
        if not generate:
            ...
            raise RuntimeError(
                'Image generation requested but MXM_GEN_IMAGE_URL / MXM_GEN_IMAGE_KEY are missing. '
                'Set env vars or create image.env from templates/image.env.example.'
            )
